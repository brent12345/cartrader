...
const onChangedLocation = () => {
    ...
    if (!isNaN(parseInt(city.value))) {
      throw createError ({
        statusCode: 400,
        message: "invalid input for city"
      })
    }
...
<template>
    <!-- CARS SIDE BAR -->
    <div class="shadow border w-64 mr-10 z-30 h-[190]">
      <!-- LOCATION START -->
          <div class="p-5 flex justify-between relative cursor-pointer border-b">
            ...
            <div class="absolute border shadow left-56 p-5 top-1 -m-1 bg-white" v-if="modal.location">
              ...
              <button class="bg-blue-400 w-full mt-2 rounded text-white p-1" @click="onChangedLocation">
                Apply
              </button>
            </div>
          </div>
          ...
          <div class="p-5 flex justify-between relative cursor-pointer border-b">
            ...
            <div class="absolute border shadow left-56 p-5 top-1 -m-1 flex justify-between flex-wrap bg-white pt-5" v-if="modal.price">
              ...
              <button class="bg-green-400 w-full mt-2 rounded text-white p-2" @click="onChangePrice">Apply</button>
            </div>
          </div>
          <!-- PRICE END -->
        </div>
